...
#     st.title('Overall Participation by Nations')
#     nations_over_time = helper.participation_nation_over_time(df)
#     fig = px.line(nations_over_time , x='Year',y='count')
#     st.plotly_chart(fig)

#     st.title('Overall Events')
#     nations_over_time = helper.total_events_over_time(df)
#     fig = px.line(nations_over_time , x='Year',y='count')
#     st.plotly_chart(fig)

#     st.title('Overall Players')
#     player_over_time = helper.atheletes_over_time(df)
#     fig = px.line(player_over_time , x='Year',y='count')
#     st.plotly_chart(fig)
...
#     sport_count = df['Sport'].unique().tolist()
#     sport_count.sort()
#     sport_count.insert(0,'Overall')

#     selected_sport = st.selectbox('Select a box' , sport_count)
...
#     country_df = helper.year_wise_medal_tally(df,selected_country)
#     fig = px.line(country_df , x='Year',y='Medal')
#     st.header(f"Medal Wins by {selected_country}")
#     st.plotly_chart(fig)
    
#     st.title(f"HeatMap of Winning for {selected_country}")
#     pt = helper.country_event_heatmap(df,selected_country)
#     fig,ax = plt.subplots(figsize=(20,20))
#     ax = sns.heatmap(pt,annot=True,cmap='Greens')
#     st.pyplot(fig)

#     st.title(f"Top Performer of {selected_country}")
#     top_ath = helper.country_wise_top_athelete(df,selected_country)
#     st.table(top_ath)
...
#     x_lists = [arr.tolist() for arr in x]
#     # Only keep sports with at least 2 unique gold medalist ages
#     filtered_x_lists = []
#     filtered_names = []
#     for arr, n in zip(x_lists, name):
#         if len(arr) > 1 and len(set(arr)) > 1:
#             filtered_x_lists.append(arr)
#             filtered_names.append(n)

#     fig = ff.create_distplot(filtered_x_lists, filtered_names, show_hist=False, show_rug=False)
...
#     men_women_df = helper.men_women_participation(df)
#     fig = px.line(men_women_df , x='Year',y = ['Male','Female'])
#     st.plotly_chart(fig)